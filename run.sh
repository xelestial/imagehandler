...
archive_bad_venv_if_needed() {
  ...
  version="$(python_version_text "$VENV_PY")"
...
[[ -x "$VENV_PY" ]] || fail "venv Python not found after setup: $VENV_PY"
python_ok "$VENV_PY" || fail "venv Python is unsupported: $(python_version_text "$VENV_PY")"
...
if [[ $# -eq 0 ]]; then
  print_workspace_hint
  log "Launching interactive menu with $VENV_PY ($(python_version_text "$VENV_PY"))"
  IMAGEHANDLER_WORKSPACE="$WORKSPACE_DIR" "$VENV_PY" -m imagehandler.cli menu
  exit $?
fi
...
This runner never relies on shell activation or PATH python. It always executes:
  .venv/bin/python
...
USAGE
    ;;
  *)
    IMAGEHANDLER_WORKSPACE="$WORKSPACE_DIR" "$VENV_PY" -m imagehandler.cli "$@"
    ;;
esac
